...
- **2025-10-13 - About & Services template migration**
  ...
  - Result: Imported the AI handoff collateral into dedicated `page-about-us.html` and `page-services.html` templates with scoped styling, CTA analytics hooks, and the native contact intake block embedded on Services. Manual Chrome DevTools sweeps on both pages confirmed layout parity and clean consoles.

  - Follow-up: Re-run the embedded contact intake block on /services/ after the next build to confirm the submission logs an `mcd_lead` entry, sends the notification email, and fires the GA4 `lead_form_view` dataLayer event. (`tests/navigation.spec.ts` now targets the new hero headline as of 2025-10-13, so the Chromium navigation suite passes again.)



## Recent Sweeps (October 2025)

- **2025-10-13 -- Contact intake pipeline**

  - Result: Built the `page-contact.html` template, contact hero pattern, and server-rendered `mccullough-digital/contact-form` block; submissions now log to the `mcd_lead` post type, send a summary email to hello@mccullough.digital, and expose dataLayer events for GA4.

  - Follow-up: Run `npm run build` and the new contact Playwright spec after bundling, verify the REST endpoint returns 200 on staging, and confirm the hello@mccullough.digital inbox receives the intake email.
