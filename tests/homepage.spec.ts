...
test.describe('Homepage smoke checks', () => {
  test('hero renders neon headline and CTA', async ({ page }) => {
    ...
    const headline = hero.getByRole('heading', { level: 1, name: /Websites that purr/i });
    await expect(headline).toBeVisible();
    const primaryCta = hero.getByRole('link', { name: 'Get Pricing & Timeline', exact: true });
    await expect(primaryCta).toBeVisible();
    const secondaryCta = hero.getByRole('link', { name: /^See Packages$/i });
