...
test.describe('Case Studies index experience', () => {
  test('spotlights the OnMark case study card', async ({ page }) => {
    ...
    const segmentChip = card.getByRole('link', { name: /Growth Sprint/i });
    ...
    const viewCaseStudy = card.getByRole('link', { name: /View case study/i });
    await expect(viewCaseStudy).toBeVisible();
    await expect(viewCaseStudy).toHaveAttribute('href', /onmark-houston-digital-transformation\/?$/);
  });
});
