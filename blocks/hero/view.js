	...
	const init = () => {
		...
		const createStarField = ( hero ) => {
			...
			const createStar = () => ( {
				...
				size:
					Math.random() *
						( CONFIG.STAR_SIZE_RANGE.max -
							CONFIG.STAR_SIZE_RANGE.min ) +
					CONFIG.STAR_SIZE_RANGE.min,
				vy:
					Math.random() *
						( CONFIG.STAR_VELOCITY_RANGE.max -
							CONFIG.STAR_VELOCITY_RANGE.min ) +
					CONFIG.STAR_VELOCITY_RANGE.min,
				twinkleSpeed:
					Math.random() *
						( CONFIG.TWINKLE_SPEED_RANGE.max -
							CONFIG.TWINKLE_SPEED_RANGE.min ) +
					CONFIG.TWINKLE_SPEED_RANGE.min,
			...
			const createShootingStar = () => ( {
				...
				len:
					Math.random() *
						( CONFIG.SHOOTING_STAR_LENGTH_RANGE.max -
							CONFIG.SHOOTING_STAR_LENGTH_RANGE.min ) +
					CONFIG.SHOOTING_STAR_LENGTH_RANGE.min,
				speed:
					Math.random() *
						( CONFIG.SHOOTING_STAR_SPEED_RANGE.max -
							CONFIG.SHOOTING_STAR_SPEED_RANGE.min ) +
					CONFIG.SHOOTING_STAR_SPEED_RANGE.min,
				size:
					Math.random() *
						( CONFIG.SHOOTING_STAR_SIZE_RANGE.max -
							CONFIG.SHOOTING_STAR_SIZE_RANGE.min ) +
					CONFIG.SHOOTING_STAR_SIZE_RANGE.min,
			} );
			...
			const repopulate = () => {
				const starCount = Math.min(
					...
					Math.max(
						50,
						Math.round(
							( state.width * state.height ) / CONFIG.STAR_DENSITY
						)
					)
				);
				...
				state.shootingStars = Array.from(
					{ length: CONFIG.MAX_SHOOTING_STARS },
					createShootingStar
				);
			};
			...
			if ( supportsResizeObserver ) {
				state.resizeObserver = new window.ResizeObserver( refresh );
			...
			if ( supportsIntersectionObserver ) {
				state.visibilityObserver = new window.IntersectionObserver(
					( entries ) => {
		...
		heroBlocks.forEach( ( hero ) => {
			...
			const allowHeadlineAnimation = hero.classList.contains(
				'has-headline-animation'
			);
