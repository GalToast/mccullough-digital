...
const defaultSecondaryButtonText =
	metadata?.attributes?.secondaryButtonText?.default ??
	__( 'View Case Studies', 'mccullough-digital' );
const defaultSecondaryButtonLink =
	metadata?.attributes?.secondaryButtonLink?.default ?? '#case-study';
...
registerBlockType( metadata.name, {
	...
	edit: function Edit( { attributes, setAttributes, clientId } ) {
		...
		const normalizedAlignment = alignmentOptions.includes(
			contentAlignment
		)
			? contentAlignment
			: 'center';
		...
		const extractNaturalWidth = ( media ) => {
			...
			return detailsWidth || fullSizeWidth || width || 0;
		};
		...
		const buildImagePresentation = () => {
			...
			if ( imageHorizontalOffset !== 0 ) {
				transformParts.push(
					`translateX(${ imageHorizontalOffset }px)`
				);
			}
				...
				<InspectorControls>
					...
					>
						...
						<RangeControl
							...
							onChange={ ( value ) =>
								setAttributes( {
									contentOffset: clampOffset( value ),
								} )
							}
							...
							allowReset
							help={ __(
								'Negative values move content UP, positive values move content DOWN',
								'mccullough-digital'
							) }
						/>
					</PanelBody>
